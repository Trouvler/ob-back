...
from account.models import User, UserProfile, AdminType, ProblemPermission, QuizPermission
from problem.models import Problem, ProblemTag, ProblemDifficulty, ProblemRuleType
from quiz.models import Quiz, QuizTag, QuizDifficulty, QuizRuleType
...
def set_quiz_display_id_prefix():
    while True:
        print("Please input a prefix which will be used in all the imported quiz's displayID")
        print(
            "For example, if your input is 'old'(no quote), the quizs' display id will be old1, old2, old3..\ninput:",
            end="")
        resp = input()
        if resp.strip():
            return resp.strip()
    ...
    if get_input_result():
        for data in users.values():
            ...
            if admin_type == AdminType.ADMIN:
                ...
                user.quiz_permission = QuizPermission.OWN
            elif admin_type == AdminType.SUPER_ADMIN:
                user.problem_permission = ProblemPermission.ALL
                user.quiz_permission = QuizPermission.ALL
            user.save()
...
def import_quizs():
    i = 0
    print("\nFind %d quizs in old data" % len(quizs))
    prefix = set_quiz_display_id_prefix()
    print("import quizs using prefix: %s? (yes/no)" % prefix)
    if get_input_result():
        default_creator = User.objects.first()
        for data in quizs:
            data["_id"] = prefix + str(data.pop("id"))
            if Quiz.objects.filter(_id=data["_id"]).exists():
                print("%s has the same display_id with the db quiz" % data["title"])
                continue
            try:
                creator_id = \
                    User.objects.filter(username=users[data["created_by"]]["username"]).values_list("id", flat=True)[0]
            ...
            data["created_by_id"] = creator_id
            data.pop("created_by")
            data["difficulty"] = QuizDifficulty.Mid
            if data["spj_language"]:
                data["spj_language"] = languages_map[data["spj_language"]]
            data["samples"] = json.loads(data["samples"])
            data["languages"] = ["C", "C++"]
            ...
            if not test_case_score:
                ...
                continue
            data["test_case_score"] = test_case_score
            data["rule_type"] = QuizRuleType.ACM
            data["template"] = {}
            data.pop("total_submit_number")
            data.pop("total_accepted_number")
            tag_ids = data.pop("tags")
            quiz = Quiz.objects.create(**data)
            quiz.create_time = data["create_time"]
            quiz.save()
            for tag_id in tag_ids:
                tag, _ = QuizTag.objects.get_or_create(name=tags[tag_id])
                quiz.tags.add(tag)
            i += 1
            print("%s imported successfully" % data["title"])
    print("%d quizs have successfully imported" % i)
...
if __name__ == "__main__":
    ...
    for obj in old_data:
        ...
        elif obj["model"] == "quiz.quiztag":
            tags[obj["pk"]] = obj["fields"]["name"]
        elif obj["model"] == "account.user":
            users[obj["pk"]] = obj["fields"]
        elif obj["model"] == "quiz.quiz":
            obj["fields"]["id"] = obj["pk"]
            quizs.append(obj["fields"])
    import_users()
    import_tags()
    import_problems()
    import_quizs()
