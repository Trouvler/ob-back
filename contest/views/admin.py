...
class ACMContestHelper(APIView):
    ...
    def get(self, request):
        ...
        for rank in ranks:
            ...
            for quiz_id, info in rank["submission_info"].items():
                if info["is_ac"]:
                    results.append({
                        "id": rank["id"],
                        "username": rank["user__username"],
    ...
    def put(self, request):
        ...
        quiz_rank_status = rank.submission_info.get(data["quiz_id"])
        if not problem_rank_status:
            return self.error("Problem id does not exist")
        if not quiz_rank_status:
            return self.error("Quiz id does not exist")
        problem_rank_status["checked"] = data["checked"]
        quiz_rank_status["checked"] = data["checked"]
...
class DownloadContestSubmissions(APIView):
    def _dump_submissions(self, contest, exclude_admin=True):
        ...
        quiz_ids = contest.quiz_set.all().values_list("id", "_id")
        id2display_id = {k[0]: k[1] for k in problem_ids}
        ac_map = {k[0]: False for k in problem_ids}

        id2display_id_quiz = {k[0]: k[1] for k in quiz_ids}
        ac_map_quiz = {k[0]: False for k in quiz_ids}
        ...
        with zipfile.ZipFile(path, "w") as zip_file:
            for user in users:
                ...
                user_ac_map_quiz = copy.deepcopy(ac_map_quiz)
