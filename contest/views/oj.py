...
class ContestRankAPI(APIView):
    ...
    def get(self, request):
        ...
        if download_csv:
            ...
            contest_quizs = Quiz.objects.filter(contest=self.contest, visible=True).order_by("_id")
            problem_ids = [item.id for item in contest_problems]
            quiz_ids = [item.id for item in contest_quizs]
            ...
            if self.contest.rule_type == ContestRuleType.OI:
                ...
                for item in range(contest_quizs.count()):
                    worksheet.write(self.column_string(5 + item) + "1", f"{contest_quizs[item].title}")
                for index, item in enumerate(data):
                    ...
                    for k, v in item["submission_info"].items():
                        ...
                        worksheet.write_string(index + 1, 4 + quiz_ids.index(int(k)), str(v))
            else:
                ...
                for item in range(contest_quizs.count()):
                    worksheet.write(self.column_string(7 + item) + "1", f"{contest_quizs[item].title}")
                ...
                for index, item in enumerate(data):
                    ...
                    for k, v in item["submission_info"].items():
                        ...
                        worksheet.write_string(index + 1, 6 + quiz_ids.index(int(k)), str(v["is_ac"]))
