...
class QuizSubmission(models.Model):
    ...
    result = models.IntegerField(db_index=True, default=JudgeStatus_quiz.PENDING)
    # 从JudgeServer返回的判题详情
    info = JSONField(default=dict)
    shared = models.BooleanField(default=False)
    # 存储该提交所用时间和内存值，方便提交列表显示
    ...
    def check_user_permission(self, user, check_share=True):
        if self.user_id == user.id or user.is_super_admin() or user.can_mgmt_all_quiz() or self.quiz.created_by_id == user.id:
            return True

        if check_share:
            if self.contest and self.contest.status != ContestStatus.CONTEST_ENDED:
                return False
            if self.quiz.share_submission or self.shared:
                return True
        return False
