...
from ..serializers import (CreateSubmissionSerializer, SubmissionModelSerializer,
                           ShareSubmissionSerializer, CreateQuizSubmissionSerializer, ShareSubmissionQuizSerializer)
from ..serializers import SubmissionSafeModelSerializer, SubmissionListSerializer, SubmissionQuizListSerializer, SubmissionQuizSafeModelSerializer, SubmissionQuizModelSerializer
...
class SubmissionQuizAPI(APIView):
    def throttling(self, request):
        # 使用 open_api 的请求暂不做限制
        auth_method = getattr(request, "auth_method", "")
        if auth_method == "api_key":
    ...
    @validate_serializer(CreateQuizSubmissionSerializer)
    @login_required
    def post(self, request):
        data = request.data
        hide_id = False
        if data.get("contest_id"):
            ...
            if error:
                return error
            contest = self.contest
            if not contest.quiz_details_permission(request.user):
                hide_id = True
    ...
    def get(self, request):
        ...
        if not submission_id:
            return self.error("Parameter id doesn't exist")
        try:
            submission = QuizSubmission.objects.select_related("quiz").get(id=submission_id)
        except QuizSubmission.DoesNotExist:
            return self.error("Submission doesn't exist")
        if not submission.check_user_permission(request.user):
            return self.error("No permission for this submission")

        if submission.quiz.rule_type == QuizRuleType.OI or request.user.is_admin_role():
            submission_data = SubmissionQuizModelSerializer(submission).data
        else:
            submission_data = SubmissionQuizSafeModelSerializer(submission).data
        # 是否有权限取消共享
        submission_data["can_unshare"] = submission.check_user_permission(request.user, check_share=False)
        return self.success(submission_data)
    ...
    def put(self, request):
        ...
        """
        try:
            submission = QuizSubmission.objects.select_related("quiz").get(id=request.data["id"])
        except QuizSubmission.DoesNotExist:
            return self.error("Submission doesn't exist")
        if not submission.check_user_permission(request.user, check_share=False):
            return self.error("No permission to share the submission")
...
class ContestSubmissionListAPI(APIView):
    ...
    def get(self, request):
        ...
        if quiz_id:
            try:
                problem = Quiz.objects.get(_id=quiz_id, contest_id=contest.id, visible=True)
            except Quiz.DoesNotExist:
                return self.error("Problem doesn't exist")
            submissions = submissions.filter(problem=problem)
...
class SubmissionQuizExistsAPI(APIView):
    def get(self, request):
        if not request.GET.get("quiz_id"):
            return self.error("Parameter error, quiz_id is required")
        return self.success(request.user.is_authenticated and
                            Submission.objects.filter(problem_id=request.GET["quiz_id"],
                                                      user_id=request.user.id).exists())
