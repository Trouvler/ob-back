...
from .models import QuizPermission
...
class quiz_permission_required(admin_role_required):
    def check_permission(self):
        if not super(quiz_permission_required, self).check_permission():
            return False
        if self.request.user.quiz_permission == QuizPermission.NONE:
            return False
        return True
...
def ensure_created_by(obj, user):
    ...
    if isinstance(obj, Problem):
        ...
        if not user.can_mgmt_all_quiz() and obj.created_by != user:
            raise e
