...
class ProfileQuizDisplayIDRefreshAPI(APIView):
    @login_required
    def get(self, request):
        profile = request.user.userprofile
        acm_quizs = profile.acm_quizs_status.get("quizs", {})
        oi_quizs = profile.oi_quizs_status.get("quizs", {})
        ids = list(acm_quizs.keys()) + list(oi_quizs.keys())
        if not ids:
            return self.success()
        display_ids = Pr.objects.filter(id__in=ids, visible=True).values_list("_id", flat=True)
        id_map = dict(zip(ids, display_ids))
        for k, v in acm_quizs.items():
            v["_id"] = id_map[k]
        for k, v in oi_quizs.items():
            v["_id"] = id_map[k]
        profile.save(update_fields=["acm_quizs_status", "oi_quizs_status"])
        return self.success()
